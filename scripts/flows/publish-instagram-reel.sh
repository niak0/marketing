#!/usr/bin/env bash
# Instagram Business hesabına Reel yayınlar.
# Not: Reels sekmesinde görünmek için video 9:16 oranında ve 5-90sn olmalı;
# bu script bunu doğrulamaz, dışındaki videolar normal video post olarak yayınlanır.
# Kullanım: ./publish-instagram-reel.sh <video_url> <caption> [--dry-run]
...
if [[ $# -lt 2 ]]; then
  echo "Kullanım: $0 <video_url> <caption> [--dry-run]" >&2
  exit 1
fi
...
echo "1/3 Reel container oluşturuluyor (media_type=REELS)..."
...
echo "2/3 Video işlenmesi bekleniyor (FINISHED bekleniyor, birkaç dakika sürebilir)..."
STATUS_CODE=""
for i in $(seq 1 60); do
  ...
  if [[ "$STATUS_CODE" == "ERROR" ]]; then
    echo "HATA: Video işlenirken hata (adım 2/3 - status ERROR döndü)." >&2
...
if [[ "$STATUS_CODE" != "FINISHED" ]]; then
  echo "HATA: Video 5dk içinde FINISHED olmadı (adım 2/3 - son durum: $STATUS_CODE)." >&2
  exit 1
fi
echo "   Durum: FINISHED"
...
echo "Yayınlandı: Reel ID $POST_ID"
